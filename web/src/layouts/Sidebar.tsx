...
export function Sidebar({ isOpen = true, onClose }: SidebarProps) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside
                className={`fixed left-0 top-0 h-screen w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-6 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                <div className="flex items-center justify-between mb-10 px-2">
                    ...
                    >
                        <LogOut size={20} className="rotate-180" /> {/* Reusing Icon for now or should import X */}
                    </button>
                </div>
